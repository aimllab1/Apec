...
async function build() {
  ...
  const aiKnowledgeDir = 'Z:/Frontend_Source/src/ai/knowledge';
  ensureDirExists(aiKnowledgeDir);

  // 1. Extract Bus Routes programmatically from FacilityDetail.jsx
  let busRoutes = [];
  try {
    const facilityDetailContent = fs.readFileSync('Z:/Frontend_Source/src/pages/FacilityDetail.jsx', 'utf8');
  ...
  let adminList = [];
  try {
    const adminDataFilePath = pathToFileURL(path.resolve('Z:/Frontend_Source/src/data/administrationData.js')).href;
  ...
  let departmentsMap = {};
  try {
    const deptRaw = fs.readFileSync('Z:/Frontend_Source/src/data/departmentsData.json', 'utf8');
