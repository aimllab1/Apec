import collegeData from '../knowledge/college.json';
import admissionsData from '../knowledge/admissions.json';
import placementsData from '../knowledge/placements.json';
import departmentsData from '../knowledge/departments.json';
...
// Load knowledge base statically
...
console.log('[LocalAIClient] Search engine initialized client-side.');
...
export function getLocalResponse(queryText, session = null) {
  ...
  if (conversationalReply) {
    ...
    return { response: conversationalReply, updatedSession: session };
  }
  ...
  console.log(`[LocalAIClient] Query: "${queryText}" | Dept: ${mentionedDept || session?.lastDepartment || 'none'} | Intent: ${intent} | FollowUp: ${isFollowUpQuery}`);
  ...
  console.log(`[LocalAIClient] Best: "${best.id}" score=${best.score.toFixed(4)}`);
  ...
  if (best.score >= HIGH) {
    ...
    return { response, updatedSession: session };
  }
  ...
  if (best.score >= MEDIUM) {
    ...
    return { response: fullResponse, updatedSession: session };
  }
  ...
  return { response: fallbackResponse, updatedSession: session };
}
